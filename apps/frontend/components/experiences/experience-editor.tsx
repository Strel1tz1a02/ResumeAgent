  ...
  return (
    <section className="space-y-4" aria-label={t('experiences.editor.title')}>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        {(['organization', 'role', 'location'] as const).map((field) => (
        ...
        <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)_auto] items-end gap-2">
          <FieldAiEntry {...entryProps('start_date')} className="min-w-0">
            <Label htmlFor="experience-start-date">{t('experiences.editor.start_date')}</Label>
            <Input
              id="experience-start-date"
              type="month"
              value={draft.start_date}
              onChange={(event) => change('start_date', event.target.value)}
              disabled={archived || saving || globalSaving || locked('start_date')}
            />
          </FieldAiEntry>
          <span className="pb-2 text-sm text-ink-soft" aria-hidden="true">
            –
          </span>
          <FieldAiEntry {...entryProps('end_date')} className="min-w-0">
            ...
            <Input
              ...
              disabled={
                archived || saving || globalSaving || draft.is_current || locked('end_date')
              }
            />
          </FieldAiEntry>
          <FieldAiEntry {...entryProps('is_current')} className="self-end" showAiStart={false}>
            <label className="flex h-10 whitespace-nowrap items-center gap-2 text-sm">
              <input
                type="checkbox"
