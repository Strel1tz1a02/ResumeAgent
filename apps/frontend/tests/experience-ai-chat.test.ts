...
describe('experience AI chat API', () => {
  ...
  it('parses fragmented text and atomic proposal SSE events', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValue(
        sseResponse([
          ...
          'delta\ndata: {"text":"你好"}\n\nevent: field_overwrite.requested\n',
          'data: {"proposal_id":4,"tool_name":"field_overwrite","proposal":{"operation":"replace","target":{"key":"background","ref_id":null},"proposed_value":"新值"}}\n\n',
        ])
      );
    vi.stubGlobal('fetch', fetchMock);
    ...
    expect(events[2].data).toMatchObject({ proposal_id: 4, tool_name: 'field_overwrite' });
  });
  ...
  it('sends one user turn and one approval resolution with caller-owned signals', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(sseResponse(['event: field_overwrite.applied\ndata: {}\n\n']));
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('extracts an authoritative experience only from business result events', () => {
    ...
    expect(eventExperienceDetail({ event: 'field_overwrite.applied', data: { experience } })).toBe(
      experience
    );
