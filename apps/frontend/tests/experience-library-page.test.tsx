...
const translate = vi.hoisted(
  () => (key: string) =>
    ({
      ...
      'experiences.editor.is_current': 'Present',
...
describe('ExperienceLibraryPage', () => {
  ...
  it('binds every evidence field to one shared evidence conversation', async () => {
    ...
    fireEvent.focus(await screen.findByLabelText('Result 8'));
    expect(screen.getAllByRole('button', { name: 'Start AI chat' })).toHaveLength(1);
    fireEvent.click(screen.getByRole('button', { name: 'Start AI chat' }));
    fireEvent.click(screen.getByRole('button', { name: 'Start' }));
