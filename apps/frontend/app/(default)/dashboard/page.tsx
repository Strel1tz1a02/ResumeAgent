...
export default function DashboardPage() {
  ...
  const totalCards = 1 + tailoredResumes.length + 4;
  ...
  return (
    <div className="space-y-6">
      ...
      <SwissGrid>
            ...
            <div className="flex-1 flex flex-col h-full">
              ...
              >
                <div className="flex min-w-0 items-start gap-1">
                  <span className="mt-0.5 shrink-0">{getStatusDisplay().icon}</span>
                ...
                {(processingStatus === 'failed' || processingStatus === 'processing') && (
                  <div
                    className="flex min-w-0 flex-col gap-2 @lg:flex-row"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <Button
                      variant="outline"
                      size="sm"
                      className="h-auto min-h-7 w-full whitespace-normal rounded-none border-black px-2 py-1 text-center text-xs @lg:w-auto"
                      onClick={handleRetryProcessing}
                      disabled={isRetrying}
                    >
                      {isRetrying
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      className="h-auto min-h-7 w-full whitespace-normal rounded-none border-red-600 px-2 py-1 text-center text-xs text-red-600 hover:bg-red-50 @lg:w-auto"
        ...
        <Link href="/experiences" className="block h-full">
          <Card variant="interactive" className="aspect-square h-full bg-secondary">
            <div className="flex h-full flex-1 flex-col justify-between">
              <div className="flex h-14 w-14 items-center justify-center border-2 border-black bg-primary text-white">
                <BookOpenText className="h-7 w-7" />
              </div>
              <div>
                <CardTitle className="text-lg uppercase">
                  {t('dashboard.experienceLibrary.title')}
                </CardTitle>
                <CardDescription className="mt-2 text-xs">
                  {t('dashboard.experienceLibrary.description')}
                </CardDescription>
              </div>
            </div>
          </Card>
        ...
        {/* 5. Evidence-grounded Resume Generation */}
        <Link href="/resume-generation" className="block h-full">
          <Card variant="interactive" className="aspect-square h-full bg-blue-50">
            <div className="flex h-full flex-1 flex-col justify-between">
              <div className="flex h-14 w-14 items-center justify-center border-2 border-black bg-blue-700 text-white">
                <Sparkles className="h-7 w-7" />
              </div>
              <div>
                <CardTitle className="text-lg uppercase">
                  {t('dashboard.resumeGeneration.title')}
                </CardTitle>
                <CardDescription className="mt-2 text-xs">
                  {t('dashboard.resumeGeneration.description')}
                </CardDescription>
              </div>
            </div>
          </Card>
