...
from app.repositories.experience_field_state_repository import ExperienceFieldStateRepository
...
class ExperienceFieldService:
    ...
    async def initialize_experience(self, item: ExperienceItem) -> None:
        ...
        await self._states.create(
            item.experience_id,
            "evidence_new",
            "complete" if item.evidence_ids else "incomplete",
        )
    ...
    async def validate_expected(
        ...
        """校验手动保存涉及字段的客户端 revision。空映射仅用于旧客户端兼容。"""
        if not expected:
            return
        for key in changed_keys:
            if key not in expected:
                raise FieldRevisionConflictError(f"missing expected revision for {key}")
            row = await self.require_state(experience_id, key)
            if row.revision != expected[key]:
                raise FieldRevisionConflictError(f"stale field revision for {key}")
    ...
    async def advance_collection(self, item: ExperienceItem) -> None:
        ...
        await self._states.advance(
            row, status="complete" if item.evidence_ids else "incomplete"
        )
    ...
    async def snapshot(
        ...
        if key in EVIDENCE_TARGET_KEYS and ref_id is not None:
            ...
            evidence = await self._evidence.get(ref_id)
            if evidence is None:
                raise FieldStateInvariantError("evidence does not exist")
