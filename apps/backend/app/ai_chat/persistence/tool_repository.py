...
class ToolCallRepository:
    ...
    async def _flush(self) -> None:
        """把数据库唯一性竞争翻译成 Runtime 可识别的存储冲突。"""
        try:
            flush = self._session.flush
            await flush()
        except IntegrityError as error:
            raise ToolPersistenceConflictError(str(error)) from error

    async def _execute(self, statement: Any) -> Any:
        """执行 SQL，并阻止 SQLAlchemy 异常越过具体存储边界。"""
        try:
            execute = self._session.execute
    ...
    async def get_by_resolution_id(
        self, thread_id: int, client_resolution_id: str
    ) -> AiChatToolCall | None:
        """查找幂等的提案审批记录。"""
        result = await self._execute(
            select(AiChatToolCall).where(
                AiChatToolCall.thread_id == thread_id,
    ...
    async def pending_results(self, thread_id: int) -> list[AiChatToolCall]:
        """返回尚未被成功模型响应消费的工具结果。"""
        result = await self._execute(
