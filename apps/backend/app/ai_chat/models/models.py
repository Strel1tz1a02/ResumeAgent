...
class AiChatConversationMemorySnapshot(Base):
    ...
    parent_snapshot_id: Mapped[int | None] = mapped_column(
        ForeignKey(
            "ai_chat_conversation_memory_snapshots.id", ondelete="CASCADE"
        ),
        nullable=True,
    )
    ...
    source_bundle_hash: Mapped[str | None] = mapped_column(
...
class AiChatConversationMemory(Base):
    ...
    active_snapshot_id: Mapped[int] = mapped_column(
        ForeignKey(
            "ai_chat_conversation_memory_snapshots.id", ondelete="RESTRICT"
        )
    )
