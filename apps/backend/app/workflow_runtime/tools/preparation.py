...
from app.workflow_runtime.tools.persistence import (
    ...
    validate_tool_call_record,
)
from app.workflow_runtime.tools.registry import ToolRegistry
from app.workflow_runtime.tools.types import ToolCall, ToolContext, ToolResult
    ...
    async def _prepare_materialized(
        ...
        async with self.store.transaction() as uow:
            ...
            if row.status != "received":
                return tool_call_from_record(row, replayed=True)
            ...
            if saved:
                ...
                await uow.rollback()
        return await load_tool_call(self.store, tool_call_id, replayed=not saved)
