...
class ExperienceWorkflow(ConversationWorkflow[ExperienceState]):
    """把通用会话输入翻译为经历领域状态与图。"""
    ...
    async def validate_request(
        ...
        if subject.type != "experience":
            raise ValueError("ExperienceWorkflow only accepts experience subjects")
        try:
    ...
    async def init_state(self, value: ConversationInput) -> ExperienceState:
    ...
    def build_graph(
        ...
        """返回由经历业务定义、尚未编译的图。"""
        return build_experience_graph(model, tools)
