...
**192 → ~444 deterministic tests**, **0 failures**. Phases 1–5 were built via parallel subagents (one per phase, strict file ownership) using the `dispatching-parallel-agents` skill; Phase 7 followed the TinyDB→SQLite migration (PRs #841 + #843).
...
| 2026-05-30 | Keep `pytest`; add `respx`, `pytest-cov`, Playwright smoke. | Existing framework is correct; fill gaps rather than replace. |
