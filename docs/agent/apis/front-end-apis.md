...
- `POST /experiences/import-text/preview` with `{ "text": string }` — uses LangChain structured output and Pydantic to parse at most 20,000 nonblank characters into an editable `ExperienceGlobalSave`. It does not create database rows.
