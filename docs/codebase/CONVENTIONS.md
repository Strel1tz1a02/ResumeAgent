...
| 项目 | 规则 | 示例 | 证据 |
|------|------|------|------|
| Python 文件 | snake_case.py | conversation_run_writer.py | apps/backend/app/ai_chat/services/conversation_run_writer.py |
| Python 函数/方法 | snake_case；异步 I/O 使用 async def | resolve | apps/backend/app/workflow_runtime/interactions.py |
| Python 类型/协议 | PascalCase；数据边界常用 dataclass/Pydantic | GraphOutcome、GraphExecutor | apps/backend/app/workflow_runtime/protocol.py；apps/backend/app/workflow_runtime/graph/driver.py |
...
| 事件名 | lower-case dotted vocabulary | run.started、interaction.requested | apps/backend/app/workflow_runtime/events.py |
...
- Conversation 与业务可以按需依赖 workflow_runtime 能力；workflow_runtime 不得反向依赖它们。
- 前端跨目录使用 @/ 别名；同目录可使用相对 import，见 apps/frontend/tsconfig.json 与 apps/frontend/lib/api/*.ts。
- 公共 Workflow 能力从 app.workflow_runtime 暴露；业务应注入所需能力，不传递聚合 Runtime 对象。

## 4) 协议与状态约定

- 事件采用 RuntimeEvent(type, run_id, sequence, payload)；payload 为对象。
- Graph 只向 Driver 产生 RuntimeEvent 或 GraphOutcome；自定义 dict 事件被拒绝。
- 外部恢复输入先经 Workflow 校验并持久化，再把 identity-only GraphResumeCommand 送给 checkpoint。
...
证据：apps/backend/app/workflow_runtime/protocol.py、graph/driver.py、runs.py、context.py。

## 5) 错误与日志

- HTTP 边界将稳定领域错误映射为 HTTP 状态；SSE 边界将未处理异常转为带稳定 code 的 run.failed，见 apps/backend/app/ai_chat/errors.py 与 streaming/sse.py。
- 持久化状态先提交，再发送 suspended/completed/interaction 事件，避免前端领先于事实源，见 workflow_runtime/interactions.py 与 services/conversation_run_writer.py。
