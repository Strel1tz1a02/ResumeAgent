...
首个版本只支持一种导入方式：用户粘贴一段自由文本经历。应用必须先将原文立即保存为
草稿，然后用户再通过 AI 问询和/或手动结构化编辑，持续修正同一条经历记录。
...
4. 导入时必须先保存原始文本，再进行任何 LLM 调用。AI 失败不能导致用户输入丢失。
...
14. 当前开发阶段不引入数据库迁移框架，也不编写迁移脚本。新表由现有 SQLAlchemy
    metadata 初始化流程直接创建。
...
| `raw_input` | Text | 否 | `""` | 用户最初输入的完整文本，保留且允许编辑 |
...
- `q`：在标题、组织、角色、原始文本、技术和标签中进行不区分大小写搜索。
- `kind`：按一种经历类型筛选。
...
2. 立即将原文保存到 `raw_input`。
3. 创建 `kind=other`、空标题、无证据、状态为 `draft` 的记录，并计算完整度。
4. 返回 `201` 和展开后的经历详情。
5. 该事务中不得调用 LLM。
...
首个版本不在数据库中保存会话。每一轮问询的权威上下文由当前已保存的经历、证据、
原始输入以及本次提交的最新回答组成。
...
4. 拒绝任何不能由 `raw_input` 与用户回答支持的组织、日期、技术、结果或指标。
...
两张表。本期不包含 Alembic、旧数据库重写或 TinyDB 兼容层。
...
- `raw_input` 和用户回答都是不可信内容，不能作为系统指令执行。
